...
const easing = {
  ...
  snappy: (t: number) => 1 - Math.pow(1 - t, 5),
};
...
export default function EchoText({
  ...
  const tintColor = typeof tint === "string" ? tint : "#A94A4A";
      ...
      {copyIndexes
        ...
        .map((index) => (
          <span
            ...
            style={{
              color: tint ? `color-mix(in srgb, ${tintColor} ${Math.min(72, 18 + index * 5)}%, ${color})` : color,
              opacity: 0,
            }}
          >
            {text}
